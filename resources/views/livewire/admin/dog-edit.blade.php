        ...
        <form wire:submit.prevent="update" class=" needs-validation" novalidate>
            <div class="row g-5 justify-content-between px-lg-2">
                <div class="col-md-4 mb-4">
                    <div wire:ignore id="placeholder">
                        @if($dog->dog_image == null)
                            <img class="img-fluid" src="{{ asset('/dist/logo/dog-placeholder.jpg') }}">
                        @else
                            <img class=" img-thumbnail" src="{{ asset('/storage/'. $dog->dog_image)  }}">
                        @endif
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="status" class="form-label">Dog Status</label>
                        @if($dog->status == 'Deceased')
                            <input wire:model="status" class="form-control" disabled type="text">
                        @else
                            <select wire:model="status" class="form-select @error('status') is-invalid @enderror" id="validationServer04" aria-describedby="validationServer04Feedback" required>
...
{{--                            </div>--}}
            ...
            <div class="d-flex justify-content-between px-lg-2">
                ...
                <div class="d-flex gap-2">
                    @if($dog->status  == 'Deceased')
                        <button type="button" wire:click="confirmDelete()" class="btn btn-danger btn-sm-block">Delete Dog</button>
                    @else
